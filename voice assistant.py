...
class assistance_gui:
    def __init__(self,root):
        ...
        self.bg = ImageTk.PhotoImage(file="W:\\Projects\\Projects\\pyqt5\\11.jpg")
        bg = Label(self.root, image=self.bg).place(x=0, y=0)

        # self.centre = ImageTk.PhotoImage(file="loading.gif")
        # left = Label(self.root, image=self.centre).place(x=100, y=100, width=400, height=400)

        # ====start button
        start = Button(self.root, text='START', font = ("times new roman", 14), command=self.start_option).place(x=130, y=480)
